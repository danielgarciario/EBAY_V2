...
DECLARE @JobName sysname = N'EBAY - Cleanup inventory history';
DECLARE @ScheduleName sysname = N'EBAY - Daily cleanup inventory history';
...
BEGIN
    EXEC [msdb].[dbo].[sp_add_job]
        @job_name = @JobName,
        @enabled = 1,
        @description = N'Deletes EBAY inventory import batches after their retention period.';
...
BEGIN
    EXEC [msdb].[dbo].[sp_add_jobstep]
        ...
        @command = N'DECLARE @DeletedBatches int;
EXEC [ebay].[CleanupInventoryHistory] @DeletedBatches = @DeletedBatches OUTPUT;';
END;
